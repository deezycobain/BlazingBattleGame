...
- Contract-v1 atlas files are PNG. Their real PNG dimensions must be exactly `frame_width * 3` by `frame_height * 2`; metadata alone is never sufficient.
...
Contract-v1 is opted into by `assets/characters/<unit>/data/atlas-contract-v1.json`. It must declare `contract_version: "blazing-battle-character-atlas-v1"`, unit ID, and uniquely named animation entries. Each entry requires animation type, `frame_count: 6`, `rows: 2`, `columns: 3`, square frame dimensions, `frame_ms`, loop behavior, `anchor_x: "center"`, `anchor_y: "feet"`, positive `combat_visual_height`, and a PNG atlas file. Basic attacks also require hit frame `0`–`5`.

Atlas paths are relative to the manifest and may target any location inside `assets/characters/<unit>/`; they may not escape the owning unit directory or point into another unit. Metadata stays in `data/`; image assets do not.

## Recommended Contract-v1 layout

```
assets/characters/<unit>/
  data/
    unit.json
    atlas-contract-v1.json
  sprites/
    idle/<unit>_idle_atlas.png
    basic/<unit>_basic_atlas.png
    special/<unit>_special_atlas.png
```

Use `<unit>_<animation>_atlas.png` for new Contract-v1 atlas files. Equivalent existing unit-local folders such as `animations/` are allowed when they retain this metadata/image separation.
