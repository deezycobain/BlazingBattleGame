...
function swapPair(pair){const s=read('S'),swap=control('swap');if(!s||!swap||s.bbRunMode!=='road'||s.ready?.ref!==pair||window.BlazingRoadCamera?.isCombatLocked?.()||swap.disabled)return;swap.click()}
