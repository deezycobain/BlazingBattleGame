...
for(const id of [SCRIPT_ID,REFINE_ID,LOOP_ID,DOCK_ID]){
...
for(const marker of [SCRIPT_ID,REFINE_ID,LOOP_ID,DOCK_ID,'runtime/ui/battle/battle-mobile-controls.js','runtime/modes/blazing-road-battle-refinements.js','runtime/combat/combat-loop-runtime.js','runtime/ui/battle/battle-dock.js','BlazingRoadCamera?.isCombatLocked?.()'])if(!html.includes(marker))throw new Error(`Battle mobile controls: missing ${marker}`);
for(const id of [SCRIPT_ID,REFINE_ID,LOOP_ID,DOCK_ID])if((html.match(new RegExp(`id=["']${id}["']`,'g'))||[]).length!==1)throw new Error(`Battle mobile controls: ${id} injection was not unique`);
await fs.writeFile(file,html);
console.log('Battle mobile controls PASS: fixed Road dock + shared HP refinement + portrait jutsu + canonical combat loop loaded after countdown/camera locking.');
