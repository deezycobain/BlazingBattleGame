...
for(const id of [SCRIPT_ID,REFINE_ID,SOUTH_SAC_ID,TURN_ID,LOOP_ID,DOCK_ID,COUNTDOWN_ID]){
...
for(const marker of [SCRIPT_ID,REFINE_ID,SOUTH_SAC_ID,TURN_ID,LOOP_ID,DOCK_ID,COUNTDOWN_ID,'runtime/ui/battle/battle-mobile-controls.js','runtime/modes/blazing-road-battle-refinements.js','runtime/modes/south-sac-boundary-refinement.js','runtime/combat/road-round-turns.js','runtime/combat/combat-loop-runtime.js','runtime/ui/battle/battle-dock.js','runtime/modes/blazing-road-countdown-polish.js','BlazingRoadCamera?.isCombatLocked?.()'])if(!html.includes(marker))throw new Error(`Battle mobile controls: missing ${marker}`);
for(const id of [SCRIPT_ID,REFINE_ID,SOUTH_SAC_ID,TURN_ID,LOOP_ID,DOCK_ID,COUNTDOWN_ID])if((html.match(new RegExp(`id=["']${id}["']`,'g'))||[]).length!==1)throw new Error(`Battle mobile controls: ${id} injection was not unique`);
await fs.writeFile(file,html);
console.log('Battle mobile controls PASS: fixed Road dock + shared HP + South Sac floor boundary + round initiative + portrait jutsu + full-viewport countdown polish + canonical combat loop.');
