...
async function runBrowser(name,type){
  ...
  try{
    ...
    failed=true;
    const state=page?await page.evaluate(()=>({diagnostics:window.__BB_DIAGNOSTICS__||[],meta:window.BB_BUILD_META||null})).catch(()=>({diagnostics:[],meta:null})):{diagnostics:[],meta:null};
