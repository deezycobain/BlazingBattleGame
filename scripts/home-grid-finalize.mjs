...
for(const marker of [STYLE_ID,'grid-template-columns:repeat(6,minmax(0,1fr))!important','grid-column:1/3!important;grid-row:3!important','grid-column:5/7!important;grid-row:2!important']){
  if(!html.includes(marker))throw new Error(`Home grid finalize: missing ${marker}`);
}

await fs.writeFile(file,html);
console.log('Home grid finalize PASS: six-track parent grid plus deterministic secondary navigation cells enforced.');
