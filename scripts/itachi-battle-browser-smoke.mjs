...
async function run(name,type){
 ...
 try{
  ...
  const contract=await page.evaluate(async()=>{
   ...
   return {registered:!!roster.Itachi,active:[...active],team:s.pairs.map(p=>front(p)?.name),pair:pair?{x:pair.x,y:pair.y}:null,idleCount:idle.length,basicCount:basic.length,jutsuCount:jutsu.length,idleLoaded:await waitLoaded(idle),basicLoaded:await waitLoaded(basic),jutsuLoaded:await waitLoaded(jutsu),markers:{basic:html.includes('animateItachiCrowStrike'),jutsu:html.includes('animateItachiTsukuyomi'),gauge:html.includes("canonicalUnit('itachi').abilities.jutsu.gauge_reduction??45"),scale:html.includes("name==='Itachi'?1.28:1"),aoe:html.includes('Itachi Tsukuyomi AoE resolution failed'),crowSilhouette:html.includes('drawCrowSilhouette'),noSwarmProjectile:!html.includes('if(travel)draw(ITACHI_CROW_SWARM'),cinematic:html.includes('impactAt=1800,holdAfterImpact=1000'),centered:html.includes('ctx.translate(W/2,H/2)'),enemyDistortion:html.includes('createRadialGradient(cx,cy,0,cx,cy,r)'),slowRedPulse:html.includes('redPulse=.30+.08')}};
  });
  if(!contract.registered||!contract.active.includes('Itachi')||!contract.team.includes('Itachi')||contract.idleCount!==6||contract.basicCount!==6||contract.jutsuCount!==6||!contract.idleLoaded||!contract.basicLoaded||!contract.jutsuLoaded||!contract.markers.basic||!contract.markers.jutsu||!contract.markers.gauge||!contract.markers.scale||!contract.markers.aoe||!contract.markers.crowSilhouette||!contract.markers.noSwarmProjectile||!contract.markers.cinematic||!contract.markers.centered||!contract.markers.enemyDistortion||!contract.markers.slowRedPulse)throw new Error(`Itachi battle contract incomplete: ${JSON.stringify(contract)}`);
  ...
  const jutsuPromise=page.evaluate(async()=>{
   ...
   const outcome=await new Promise(resolve=>{
    ...
    setTimeout(()=>{clearInterval(sample);resolve({timeout:true,elapsed:performance.now()-start})},4300);
   });
  ...
  if(jutsuResult.error||jutsuResult.timeout||!jutsuResult.sawOverlay||!jutsuResult.sawMandala||!jutsuResult.sawTarget||!aoeDamaged||!gaugesAtImpact||jutsuResult.elapsed<2700)throw new Error(`Tsukuyomi failed: ${JSON.stringify(jutsuResult)}`);
  if(errors.length)throw new Error(`pageerror: ${errors.join(' | ')}`);
  console.log(`Itachi battle smoke PASS (${name}): single black-crow projectile, 2.8s screen-centered Tsukuyomi, enemy-side AoE distortion, multi-enemy damage, and 45 gauge suppression verified.`);
  await context.close();
