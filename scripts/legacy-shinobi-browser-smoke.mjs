...
for(const [browserName,browserType] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  const cleanReveal=await page.evaluate(()=>{
    ...
    return {src:image?.getAttribute('src')||'',name:(document.querySelector('#summonPullScreen .bb-card-nameplate')||document.querySelector('#summonPullScreen .pullName'))?.textContent?.trim()||'',fit:style?.objectFit||'',transform:style?.transform||'',legacyClass:wrap?.classList.contains('bb-legacy-full-card')||false,fallback:wrap?.classList.contains('bb-source-art-fallback')||false,wrapped:!!image?.closest('.bb-legacy-art-frame'),mask:!!document.querySelector('#summonPullScreen .bb-legacy-name-mask'),element:element?.dataset.element||'',unitClass:unitClass?.textContent?.trim()||'',origin:origin?.textContent?.trim()||'',oldCrop:[...document.querySelectorAll('#summonPullScreen img')].map(img=>img.getAttribute('src')||'').filter(src=>/legacy_summon_art\.png/i.test(src))};
  });
  await page.evaluate(()=>{if(window.__bbLegacyOriginalRandom)Math.random=window.__bbLegacyOriginalRandom});
  if(!/\/kakashi\/cards\/legacy_of_shinobi_card\.webp$/.test(cleanReveal.src)||cleanReveal.name!=='KAKASHI'||cleanReveal.fit!=='contain'||cleanReveal.transform!=='none'||!cleanReveal.legacyClass||!cleanReveal.fallback||cleanReveal.wrapped||cleanReveal.mask||cleanReveal.element!=='lightning'||cleanReveal.unitClass!=='TACTICIAN'||cleanReveal.origin!=='NARUTO'||cleanReveal.oldCrop.length)throw new Error('Kakashi reveal is not using direct full-card/no-crop + element/class/origin presentation: '+JSON.stringify(cleanReveal));
  ...
  const cleanResult=await page.evaluate(()=>{
    ...
    return {src:image?.getAttribute('src')||'',name:card?.querySelector('.charName')?.textContent?.trim()||'',fit:style?.objectFit||'',transform:style?.transform||'',legacyClass:card?.classList.contains('bb-legacy-full-card')||false,fallback:card?.classList.contains('bb-source-art-fallback')||false,wrapped:!!image?.closest('.bb-legacy-art-frame'),mask:!!card?.querySelector('.bb-legacy-name-mask'),oldCrop:[...document.querySelectorAll('#pullResultsGrid img')].map(img=>img.getAttribute('src')||'').filter(src=>/legacy_summon_art\.png/i.test(src))};
  });
