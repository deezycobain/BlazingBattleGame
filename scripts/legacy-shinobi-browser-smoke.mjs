...
for(const [browserName,browserType] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  // Regression: Legacy reveal/results must use the restored name-free presentation art.
  // The clean art fills the owned card frame; baked source-name cards are forbidden at runtime.
  ...
  await page.waitForFunction(()=>/\/kakashi\/cards\/legacy_summon_art\.png$/.test(String(document.querySelector('#summonPullScreen .summonedTradingCard')?.getAttribute('src')||'')),null,{timeout:10000});
  const cleanReveal=await page.evaluate(()=>{
    const image=document.querySelector('#summonPullScreen .summonedTradingCard'),wrap=document.getElementById('pullCardWrap'),style=image?getComputedStyle(image):null;
    const element=document.querySelector('#summonPullScreen .bb-card-elementplate'),unitClass=document.querySelector('#summonPullScreen .bb-card-classplate'),origin=document.querySelector('#summonPullScreen .bb-card-originplate');
    return {src:image?.getAttribute('src')||'',name:(document.querySelector('#summonPullScreen .bb-card-nameplate')||document.querySelector('#summonPullScreen .pullName'))?.textContent?.trim()||'',fit:style?.objectFit||'',transform:style?.transform||'',legacyClass:wrap?.classList.contains('bb-legacy-full-card')||false,fallback:wrap?.classList.contains('bb-source-art-fallback')||false,presentation:wrap?.dataset.bbArtPresentation||'',wrapped:!!image?.closest('.bb-legacy-art-frame'),mask:!!document.querySelector('#summonPullScreen .bb-legacy-name-mask'),element:element?.dataset.element||'',unitClass:unitClass?.textContent?.trim()||'',origin:origin?.textContent?.trim()||'',namedArt:[...document.querySelectorAll('#summonPullScreen img')].map(img=>img.getAttribute('src')||'').filter(src=>/legacy_of_shinobi_card|wong_fei_hung_refresh/i.test(src))};
  });
  await page.evaluate(()=>{if(window.__bbLegacyOriginalRandom)Math.random=window.__bbLegacyOriginalRandom});
  if(!/\/kakashi\/cards\/legacy_summon_art\.png$/.test(cleanReveal.src)||cleanReveal.name!=='KAKASHI'||cleanReveal.fit!=='cover'||cleanReveal.transform!=='none'||!cleanReveal.legacyClass||cleanReveal.fallback||cleanReveal.presentation!=='clean-fill'||cleanReveal.wrapped||cleanReveal.mask||cleanReveal.element!=='lightning'||cleanReveal.unitClass!=='TACTICIAN'||cleanReveal.origin!=='NARUTO'||cleanReveal.namedArt.length)throw new Error('Kakashi reveal is not using clean full-fill art + element/class/origin presentation: '+JSON.stringify(cleanReveal));
  ...
  const cleanResult=await page.evaluate(()=>{
    const card=document.querySelector('#pullResultsGrid .pullCard'),image=card?.querySelector('.resultTradingCard'),style=image?getComputedStyle(image):null;
    return {src:image?.getAttribute('src')||'',name:card?.querySelector('.charName')?.textContent?.trim()||'',fit:style?.objectFit||'',transform:style?.transform||'',legacyClass:card?.classList.contains('bb-legacy-full-card')||false,fallback:card?.classList.contains('bb-source-art-fallback')||false,presentation:card?.dataset.bbArtPresentation||'',wrapped:!!image?.closest('.bb-legacy-art-frame'),mask:!!card?.querySelector('.bb-legacy-name-mask'),namedArt:[...document.querySelectorAll('#pullResultsGrid img')].map(img=>img.getAttribute('src')||'').filter(src=>/legacy_of_shinobi_card|wong_fei_hung_refresh/i.test(src))};
  });
  if(!/\/kakashi\/cards\/legacy_summon_art\.png$/.test(cleanResult.src)||String(cleanResult.name||'').toUpperCase()!=='KAKASHI'||cleanResult.fit!=='cover'||cleanResult.transform!=='none'||!cleanResult.legacyClass||cleanResult.fallback||cleanResult.presentation!=='clean-fill'||cleanResult.wrapped||cleanResult.mask||cleanResult.namedArt.length)throw new Error('Kakashi results card is not using clean full-fill presentation: '+JSON.stringify(cleanResult));
  ...
  const tenPull=await page.evaluate(()=>{
    ...
    cards.forEach(card=>{const top=card.offsetTop,img=card.querySelector('.resultTradingCard'),fighter=card.dataset.fighter||'',src=img?.getAttribute('src')||'',expected=window.BlazingProgression?.summonArt?.(fighter)||'';rows.set(top,(rows.get(top)||0)+1);if(!fighter||!expected||!src.endsWith(expected))mismatches.push({fighter,src,expected})});
  ...
  const badRuntime=runtime.filter(x=>{
   ...
   const expectedIdleResolved=drunken?12:6;
   const expectedAttackResolved=6;
   return x.idleCount!==6||x.basicCount!==6||x.idleResolved!==expectedIdleResolved||x.attackResolved!==expectedAttackResolved||x.idleBroken||x.basicBroken||x.idleLandscape||x.basicLandscape||!normalized(x.idleDims)||!normalized(x.basicDims)||!/\/sprites\/runtime\/idle\/frame_01\.png\?legacySpriteAudit=v4$/.test(x.idleSrc)||!/\/sprites\/runtime\/attack\/basic\/frame_01\.png\?legacySpriteAudit=v4$/.test(x.basicSrc);
  });
  ...
  if(JSON.stringify(drunkenCadence.idle)!==JSON.stringify(['01','01','02','02','02','03','04','04','05','05','06','06'])||JSON.stringify(drunkenCadence.basic)!==JSON.stringify(['01','02','03','04','05','06']))throw new Error('Wong Fei-Hung drunken frame cadence invalid: '+JSON.stringify(drunkenCadence));
