...
async function run(name,type){
  ...
  try{
    ...
    if(contract.version!=='6.8.0-itachi')throw new Error(`wrong runtime version: ${JSON.stringify(contract)}`);
    ...
    const speeds=loaded.rings.map(r=>Number.parseFloat(r.duration)||0),tiered=speeds.every((value,index)=>index===0||value<speeds[index-1])&&speeds[0]<=16.2&&speeds[1]<=10.7&&speeds[2]<=6.4&&speeds[3]<=3.7;
    ...
    const snapDurations=loaded.rings.map(r=>Number.parseFloat(r.shellDuration)||9),snapOk=loaded.rings.every(r=>r.shellName.includes('bbItachiRingEngage'))&&snapDurations[0]>=.76&&snapDurations[1]>=.72&&snapDurations[2]>=.66&&snapDurations[3]>=.58;
    ...
    console.log(`Legendary Itachi summon smoke PASS (${name}): four-ring kinetic cog starts ${JSON.stringify(ringStarts.map(s=>s.index))}, high-travel gear engagement, visibly accelerating alternating inner rotation, floating Sharingan orbit removed, uniform 5% ring shrink, one-second red-flicker shadow hold, crows-before-Itachi reveal, 8.5s handoff, smoke-backed gear intro, blackout fade-back, and full-background card art verified.`);
