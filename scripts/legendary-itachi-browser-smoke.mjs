...
const EXPECTED_TIMES={ignite:0,rings:1050,orbit:2440,stop:3300,silhouette:3650,reveal:5700,settle:6800,handoff:8500};
...
async function run(name,type){
  ...
  try{
    ...
    if(contract.version!=='6.12.0-itachi')throw new Error(`wrong runtime version: ${JSON.stringify(contract)}`);
    ...
    await page.evaluate(()=>{
      ...
      scene?.addEventListener('animationstart',event=>{
        ...
        if(!shell||(!event.animationName.startsWith('bbItachiRingTripleSnap')&&!event.animationName.startsWith('bbItachiRingDoubleSnap')&&!event.animationName.startsWith('bbItachiRingSmoothIn')))return;
    ...
    const loaded=await page.evaluate(()=>{
      ...
      const stageWidth=scene.querySelector('.bb-itachi-stage-vfx')?.getBoundingClientRect()?.width||0;const rings=ringSelectors.map(selector=>{const node=scene.querySelector(selector),shell=node?.closest('.bb-itachi-ring-shell'),box=shell?.getBoundingClientRect(),nodeBox=node?.getBoundingClientRect(),style=node?getComputedStyle(node):null,shellStyle=shell?getComputedStyle(shell):null;return {selector,cx:box?box.left+box.width/2:null,cy:box?box.top+box.height/2:null,width:style?Number.parseFloat(style.width)||node?.offsetWidth||0:0,widthPct:stageWidth&&nodeBox?nodeBox.width/stageWidth*100:0,duration:style?.animationDuration||'',name:style?.animationName||'',shellName:shellStyle?.animationName||'',shellDuration:shellStyle?.animationDuration||'',shellDelay:shellStyle?.animationDelay||''}});
      return {
        count:images.length,
    ...
    const snapDurations=loaded.rings.map(r=>Number.parseFloat(r.shellDuration)||9),snapOk=loaded.rings[0].shellName.includes('bbItachiRingTripleSnap')&&loaded.rings[1].shellName.includes('bbItachiRingDoubleSnap')&&loaded.rings[2].shellName.includes('bbItachiRingSmoothIn')&&loaded.rings[3].shellName.includes('bbItachiRingSmoothIn')&&snapDurations[0]>=.90&&snapDurations[0]<=.94&&snapDurations[1]>=.76&&snapDurations[1]<=.80&&snapDurations[2]>=1.03&&snapDurations[2]<=1.07&&snapDurations[3]>=1.10&&snapDurations[3]<=1.14;
    ...
    console.log(`Legendary Itachi summon smoke PASS (${name}): four-ring kinetic cog starts ${JSON.stringify(ringStarts.map(s=>s.index))}, three-beat outer snap, two-beat second-ring snap, then two smooth inner transitions into slower tiered alternating rotation, floating Sharingan orbit removed, uniform 5% ring shrink, one-second red-flicker shadow hold, crows-before-Itachi reveal, 8.5s handoff, smoke-backed gear intro, blackout fade-back, and full-background card art verified.`);
