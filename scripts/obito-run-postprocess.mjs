...
if(!html.includes("actor.name==='Obito'&&S.drag&&S.ready?.ref?.name===actor.name&&Number.isFinite(S.dragFacing)")){
...
for(const marker of [
  ...
  'previousDragX=v?.lastX??pt.x',
  "actor.name==='Obito'&&S.drag&&S.ready?.ref?.name===actor.name"
])if(!html.includes(marker))fail('missing final marker '+marker);

await fs.writeFile(file,html);
console.log('Obito run integration PASS: picked-up Obito loops authored run frames, follows drag direction, and returns to idle on release.');