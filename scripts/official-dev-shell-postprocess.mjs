...
html=html.slice(0,body)+`<script id="${ECONOMY_ID}" src="runtime/modes/battle-economy.js"></script><script id="${RESULTS_ID}" src="runtime/ui/battle/match-results.js"></script><script id="${HOME_COMPAT_ID}" src="runtime/ui/home/home-approved-compat.js"></script>`+html.slice(body);

for(const marker of [STYLE_ID,ECONOMY_ID,RESULTS_ID,HOME_COMPAT_ID,'S.bbVictoryReward','BlazingEconomy.awardVictory','home-approved-compat.js'])if(!html.includes(marker))throw new Error(`Official dev shell: missing ${marker}`);
await fs.writeFile(file,html);
console.log('Official dev shell PASS: approved Home v4 bridge, Battle Marks rewards, and post-match return controls integrated.');
