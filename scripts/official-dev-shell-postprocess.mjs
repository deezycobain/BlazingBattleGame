...
html=html.slice(0,body)+`<script id="${ECONOMY_ID}" src="runtime/modes/battle-economy.js"></script><script id="${RESULTS_ID}" src="runtime/ui/battle/match-results.js"></script><script id="${HOME_COMPAT_ID}" src="runtime/ui/home/home-approved-compat.js"></script><script id="${HOME_LIVE_ID}" src="runtime/ui/home/home-live-polish.js"></script><script id="${HOME_V8_ID}" src="runtime/ui/home/home-v8-runtime.js"></script>`+html.slice(body);

for(const marker of [STYLE_ID,ECONOMY_ID,RESULTS_ID,HOME_COMPAT_ID,HOME_LIVE_ID,HOME_V8_ID,'S.bbVictoryReward','BlazingEconomy.awardVictory','home-approved-compat.js','home-live-polish.js','home-v8-runtime.js'])if(!html.includes(marker))throw new Error(`Official dev shell: missing ${marker}`);
await fs.writeFile(file,html);
console.log('Official dev shell PASS: approved Home compatibility, v7 parallax, v8 mockup layout/profile gate, Battle Marks rewards, and post-match return controls integrated.');
