...
html=html.replace(/<style\b[^>]*id=["']bb-progression-style["'][^>]*>[\s\S]*?<\/style>/gi,'').replace(/<script\b[^>]*id=["']bb-progression-runtime["'][^>]*>[\s\S]*?<\/script>/gi,'').replace(/<script\b[^>]*id=["']bb-progression-visual-hotfix["'][^>]*>[\s\S]*?<\/script>/gi,'').replace(/<script\b[^>]*id=["']bb-awakening-language["'][^>]*>[\s\S]*?<\/script>/gi,'');
...
const bodyAt=html.toLowerCase().lastIndexOf('</body>');html=html.slice(0,bodyAt)+`<script id="bb-progression-runtime">${js}<\/script><script id="bb-awakening-language">${awakeningLanguage}<\/script>`+html.slice(bodyAt);
for(const marker of ['RESONANCE FORGE','blazing.progression.v1','MAX_RESONANCE=5','STAT_BUDGET=12','summonEmbers=999999','spendEmbers=function()','SHINY AWAKENED','KEEP CURRENT','ACCEPT NEW','CORE RESONANCE BANNER','SKIP TO RESULTS','bb-shiny-awakening','current_collection_card.png','senku_card.jpeg','FORGE_ART','SHINY_CUTOUT','SHINY_POPOUT_PROFILE','full_art_absolute_zero_v2.jpeg','full_art_cosmic_wish.jpeg','current_collection_art.jpg','shiny_foreground_cutout_v2.webp','shiny_foreground_cutout_v1.webp','forgeArtDepth','forgePopout','hasPopout','forgeHoloTexture','bbGoldFlow','overflow-y:auto!important','PULL${pulls.length===1','fitForgeArtwork','--forge-art-ratio','--forge-art-max','syncRevealArt','syncResultArt','dataset.popoutProfile','data-popout-profile','bb-awakening-language','AWAKENING FORGE','CORE AWAKENING','BlazingAwakeningLanguage'])if(!html.includes(marker))throw new Error(`Progression pass missing ${marker}`);
await fs.writeFile(file,html);
console.log('Progression pass applied: inventory-themed summons, canonical result art, single-layer profile-driven Shiny pop-outs, skippable reveals, Shiny-to-Forge routing, persistent progression, and Awakening presentation language.');
