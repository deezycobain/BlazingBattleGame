...
const replacement=`const bbRoadDepthY=ctx.getTransform?.().f;
 const bbRoadDepthScale=(S.bbRunMode==='road'&&Number.isFinite(bbRoadDepthY))
  ...
  : 1;
 const bbUnitPresentationScale=name==='Itachi'?1.28:1;
 ctx.scale(directionalFlip*scale*activePulse*bbRoadDepthScale*bbUnitPresentationScale,scale*activePulse*bbRoadDepthScale*bbUnitPresentationScale);`;
...
for(const marker of ['bbRoadDepthY','bbRoadDepthScale','bbUnitPresentationScale','BlazingRoadContent?.visualScaleForY'])if(!html.includes(marker))throw new Error(`Road perspective: built shell missing ${marker}`);
