...
const KEY='bb:sanctuary:first-bloom:v4';
const EXPECTED_STAGE=[0,1,2,3,5,6,6];
const EXPECTED_HARMONY=[0,0,25,50,75,100,100];

async function run(name,type){
 let browser,page,errors=[];
 try{
  console.log(`Sanctuary V4 smoke START (${name}) -> ${BASE}`);
  ...
  const harmony=s=>Object.values(s.gardenApplied||{}).filter(Boolean).length*25;
  ...
  const exclusiveSnapshot=()=>page.evaluate(()=>{
   ...
   return {
    ...
    retiredSignatureNodes:['nameTemplateLayer','gardenSignature'].filter(id=>document.getElementById(id)),
   };
  });
  const assertExclusive=async(label,expected={})=>{
   ...
   if(shot.retiredSignatureNodes.length)throw new Error(`${label}: retired Signature nodes still mounted: ${shot.retiredSignatureNodes.join(',')}`);
   for(const category of ['rootBase','trunk','canopy','blossom','pattern','motif','stones','fx']){
  ...
  if(geometry.scroll>geometry.vw+1||geometry.scene.l<0||geometry.scene.r>geometry.vw+1||geometry.drawer.t<geometry.scene.b-2||geometry.potLayer)throw new Error(`V4 mobile compositor geometry invalid: ${JSON.stringify(geometry)}`);
  ...
  if(!devStatus.includes('First Bloom Rebuild')||!devStatus.includes('canonical tree 1536x1536')||!devStatus.includes('one asset per category'))throw new Error('DEV status does not describe exclusive V4 rebuild');
  for(let n=1;n<=6;n++){
   await page.locator(`[data-dev="preset-${n}"]`).click();
   await closeDev();
   await waitArt();
   const s=await state();
   if(s.schema!==4||s.treeStage!==EXPECTED_STAGE[n]||harmony(s)!==EXPECTED_HARMONY[n])throw new Error(`preset ${n} state mismatch: ${JSON.stringify(s)}`);
   if('signature' in (s.gardenApplied||{})||'nameLayout' in s)throw new Error(`preset ${n} retained retired Signature state: ${JSON.stringify(s)}`);
   ...
   if(n===4&&(s.canopyType!=='jade'||s.blossomVariant!=='blue'||s.rakePattern!=='spiral_wind'||s.motif!=='shinobi_seal_mandala'||s.stoneLayout!=='mountain'))throw new Error(`preset 4 is not deterministic: ${JSON.stringify(s)}`);
   if(n===5&&(s.rakePattern!=='flowing_river'||s.motif!=='lotus_bloom_bonus'||s.stoneLayout!=='riverbank'))throw new Error(`preset 5 is not deterministic: ${JSON.stringify(s)}`);
   ...
   if([1,4,5].includes(n)){
    ...
    await page.screenshot({path:`test-artifacts/sanctuary-v4-${name}-preset-${n}.png`,fullPage:true});
   }
   await openDev();
  }
  ...
  if(await page.locator('[data-design-section="signature"]').count())throw new Error('retired Signature tab is still visible');
  ...
  await waitArt();
  after=await state();
  if(harmony(after)!==75||'signature' in after.gardenApplied)throw new Error(`three garden elements should resolve to 75% without Signature: ${JSON.stringify(after)}`);
  ...
  await page.locator('[data-design-section="finish"]').click();
  before=await state();
  await page.locator('[data-apply="refine"]').click();
  after=await state();
  if(harmony(after)!==100||!after.gardenApplied.refine||after.resources.spiritWater!==before.resources.spiritWater-1)throw new Error(`final refinement did not complete four-step Harmony: ${JSON.stringify(after)}`);
  await fs.mkdir('test-artifacts',{recursive:true});
  await page.screenshot({path:`test-artifacts/sanctuary-v4-${name}-design-exclusive.png`,fullPage:true});
  ...
  await page.screenshot({path:`test-artifacts/sanctuary-v4-${name}-completion-exclusive.png`,fullPage:true});

  await page.reload({waitUntil:'domcontentloaded'});
  await page.waitForFunction(key=>JSON.parse(localStorage.getItem(key)||'null')?.schema===4,KEY);
  after=await state();
  if(!after.completed||!after.completionClaimed||after.grove.length<1)throw new Error('V4 completion did not persist after reload');
  await waitArt();
  ...
  await fs.mkdir('test-artifacts',{recursive:true});
  await page.screenshot({path:`test-artifacts/sanctuary-v4-${name}-final.png`,fullPage:true});
  await page.locator('#homeButton').click();
  await page.locator('#bbHomeApproved [data-nav="sanctuary"]').waitFor({state:'visible'});
  if(errors.length)throw new Error(`browser errors: ${errors.join(' | ')}`);
  await context.close();
  console.log(`Sanctuary V4 smoke PASS (${name}): Signature retired, four-step Harmony, one-per-category rendering, deterministic presets, offset stones, Design exclusivity, sequenced ceremony FX, Grove, persistence, Seal summon`);
  return true;
 }catch(e){
  try{
   await fs.mkdir('test-artifacts',{recursive:true});
   await page?.screenshot({path:`test-artifacts/sanctuary-v4-${name}-FAIL.png`,fullPage:true});
   ...
   await fs.writeFile(`test-artifacts/sanctuary-v4-${name}-FAIL.json`,JSON.stringify({error:String(e?.stack||e),browserErrors:errors,dump},null,2));
  }catch{}
  console.error(`Sanctuary V4 smoke FAIL (${name}):`,e);
  if(errors.length)console.error(`Sanctuary V4 browser errors (${name}): ${errors.join(' | ')}`);
  return false;
 }finally{
