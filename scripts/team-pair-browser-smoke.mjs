...
for(const [name,type] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  if(artContract.tagged<6||!artContract.wong||artContract.wong.kind!=='body'||artContract.wong.legacy!=='true'||artContract.wong.fit!=='contain'||artContract.wong.transform!=='none'||!/\/jackie_chan\/sprites\/runtime\/idle\/frame_01\.png/i.test(artContract.wong.src)||artContract.oldCrop.length||(artContract.wongMask&&artContract.wongMask!=='none')||!/Wong Fei-Hung/i.test(artContract.text)||/Jackie Chan/i.test(artContract.text))throw new Error('team editor clean legacy presentation fallback invalid: '+JSON.stringify(artContract));
