...
replaceOne('runBasicAttack=animateLunge;basicTarget=to;',"runBasicAttack=au.name==='Tyler'?async(...args)=>{await TYLER_BODY_RUNTIME.basic.readyPromise;return animateLunge(...args)}:animateLunge;basicTarget=au.name==='Tyler'&&Math.hypot(to.x-from.x,to.y-from.y)<40?{x:from.x+dx/len*40,y:from.y+dy/len*40}:to;",'Tyler animation-ready basic attack and minimum solo lunge distance');
replaceOne('let start=performance.now(),dur=175,backDur=145;',"let start=performance.now(),dur=unitName==='Tyler'?360:175,backDur=unitName==='Tyler'?340:145,lungeHold=unitName==='Tyler'?100:65;",'Tyler readable lunge timing');
replaceOne("anim.attackPose[unitName]={kind:attackKind,start,duration:dur+150};","anim.attackPose[unitName]={kind:attackKind,start,duration:dur+lungeHold+backDur};",'Tyler full-cycle attack pose timing');
replaceOne('},65)\n }\n requestAnimationFrame(go)','},lungeHold)\n }\n requestAnimationFrame(go)','Tyler lunge impact hold');
...
const runtime=String.raw`const TYLER_BODY_RUNTIME=(()=>{
 ...
 function buildSheet(src,columns,rows,count,contentTop,sequence,canvasW=420,insetOverride=null){
  const baseFrames=Array.from({length:count},()=>new Image());let finishReady;const readyPromise=new Promise(resolve=>{finishReady=resolve});const state={frames:[],ready:false,loaded:0,error:false,sourceFrames:count,readyPromise};const sheet=new Image();
  sheet.addEventListener('load',()=>{try{const usableH=Math.max(1,sheet.naturalHeight-contentTop),cellW=sheet.naturalWidth/columns,cellH=usableH/rows,inset=insetOverride??Math.max(8,Math.round(Math.min(cellW,cellH)*.025));for(let index=0;index<count;index++){
   ...
   const out=document.createElement('canvas');out.width=canvasW;out.height=CANVAS_H;const o=out.getContext('2d');if(maxX>=minX&&maxY>=minY){const padX=Math.max(8,Math.round((maxX-minX+1)*.03)),padY=Math.max(10,Math.round((maxY-minY+1)*.035)),cx=Math.max(0,minX-padX),cy=Math.max(0,minY-padY),cr=Math.min(cell.width,maxX+1+padX),cb=Math.min(cell.height,maxY+1+padY),bw=cr-cx,bh=cb-cy,scale=Math.min((canvasW*.94)/bw,(CANVAS_H*.92)/bh),dw=bw*scale,dh=bh*scale;o.drawImage(cell,cx,cy,bw,bh,(canvasW-dw)/2,FOOT_Y-dh,dw,dh);}
   const frame=baseFrames[index];frame.addEventListener('load',()=>{state.loaded++;if(state.loaded===count){state.frames=sequence.map(i=>baseFrames[i]||baseFrames[0]);state.ready=true;finishReady(true);}},{once:true});frame.addEventListener('error',()=>{state.error=true;finishReady(false);},{once:true});frame.src=out.toDataURL('image/png');
  }}catch(err){state.error=true;finishReady(false);console.error('Tyler sheet processing failed:',err);}},{once:true});sheet.addEventListener('error',()=>{state.error=true;finishReady(false);console.error('Tyler sheet failed to load:',src);},{once:true});sheet.src=src;return state;
 }
 return Object.freeze({
  idle:buildSheet('${idlePath}',5,1,5,0,[0,1,2,3,4,3,2,1],420,0),
  basic:buildSheet('${basicPath}',4,2,8,100,[0,0,1,2,3,4,5,6,7,7],560)
 });
})();`;
...
for(const marker of ["['crimson','subzero','lebee','senku','tyler','anubis']","['Crimson','Sub-Zero','Lebee','Senku','Tyler']","['Tyler','Lebee','Sub-Zero']",'activeTeam.v4',idlePath,basicPath,"[0,1,2,3,4,3,2,1]","[0,0,1,2,3,4,5,6,7,7]",'pruneTinyComponents','bgDistance','CANVAS_H*.92','canvasW=420','420,0','560',"name==='Tyler'?230:185","au.name==='Tyler'?async(...args)=>{await TYLER_BODY_RUNTIME.basic.readyPromise;return animateLunge(...args)}:animateLunge","au.name==='Tyler'&&Math.hypot(to.x-from.x,to.y-from.y)<40","unitName==='Tyler'?360:175","unitName==='Tyler'?340:145","unitName==='Tyler'?100:65",'duration:dur+lungeHold+backDur','},lungeHold)','readyPromise','finishReady(true)'])if(!html.includes(marker))throw new Error(`Tyler playable integration: final shell missing ${marker}`);
await fs.writeFile(file,html);console.log('Tyler playable integration applied: Tyler basic waits for source-sheet readiness on first use, then uses the authored attack animation consistently.');
