...
for(const marker of ["['crimson','subzero','lebee','senku','tyler','anubis']","['Crimson','Sub-Zero','Lebee','Senku','Tyler']","['Tyler','Lebee','Sub-Zero']",'activeTeam.v4',idlePath,basicPath,"idle:buildSheet('${idlePath}',5,1,5,100)","basic:buildSheet('${basicPath}',4,2,8,100)",'edgeBackground'])if(!html.includes(marker))throw new Error(`Tyler playable integration: final shell missing ${marker}`);
