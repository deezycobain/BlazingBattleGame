...
replaceUnique(/function renderForge\(message=''\)\{[\s\S]*?\n\}\n(?=function toggleLock)/,`function renderForge(message=''){
 ...
 document.getElementById('forgeRoster').innerHTML=forgeFighters().map(name=>{const x=P?.unit?.(name)||{level:1,awakening:0,copies:0,shiny:false};return \`<button class="forgeFighter \${name===selected?'active':''} \${x.shiny?'maxed':''}" data-fighter="\${name}">\${name}<small>\${x.shiny?'SHINY • ':''}LV.\${x.level} • A\${x.awakening}/5 • \${x.copies} COP\${x.copies===1?'Y':'IES'}</small></button>\`}).join('');
...
replaceLiteralUnique('LEGACY OF THE SHINOBI • 12 UNIT DEV BANNER • FREE TEST PULLS','LEGACY OF THE SHINOBI • 12 UNIT DEV BANNER • FREE TEST PULLS • DUPLICATES BANKED','Summon development badge');
...
for(const marker of ['bb-unit-progression-runtime','runtime/progression/unit-progression.js','COPY +1','S.bbVictoryXp','awardBattleXp','REROLL •','FORGE_REROLL_','BLAZING COINS</small>','12 UNIT DEV BANNER • FREE TEST PULLS • DUPLICATES BANKED','bb-progression-economy-style','bb-progression-economy-bridge','Math.min(100'])if(!html.includes(marker))throw new Error(`Unit progression: missing ${marker}`);
