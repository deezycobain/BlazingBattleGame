...
for(const marker of ['.bb-results-road','.bb-road-node-track','.bb-road-next','.bb-road-carry-grid','.bb-results-xp-grid','.bb-results-xp-unit.ready','.bb-results-actions.three']){
  assert(resultsCss.includes(marker),`Road results/progression CSS is missing ${marker}`);
}

console.log('Blazing Road PASS: five WebP maps, 10-stage map rotation, persistent HP/chakra, defeats, carry-forward, next-encounter intermission, per-unit battle progression/Forge routing, legacy saves, failure state, and local persistence verified.');
