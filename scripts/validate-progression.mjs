...
const runtimeMarkers=[
 ...
 'forgeArtRequestedKey','artSwitching','aria-busy','function forgeFighters()','canForge','summon_art_clean','presentation_art'
];
...
for(const marker of [
 "const VERSION='6.17.0-itachi-long-rings'",'portal:0','circleSlow:520','circleFast:980','cardEnter:1500','flip:2020','resolve:2670','done:3660',
 ...
 'await prepareVfx(refs)','await waitForPortalAnimation(refs)','bb-cinematic-preparing .pullCardWrap::before','bbPortalOpen .75s','bbOrnateRingBuild .90s','bbOuterRingCharge 1.05s','bbEnergyRingCharge .76s','bbMotionCardsBridge .86s','bbFlipShadowBeat .46s','bbFlipFrameAura 1.18s','bbFlipSlash .46s','bbCardBackEnter .30s','bbPhysicalCardFlip .65s linear','bbResolveFlash .46s','bbResolveImpact .64s','bbResolveParticles .82s','rotateY(180deg)','@media(prefers-reduced-motion:reduce)'
])if(!cinematicJs.includes(marker)&&!cinematicCss.includes(marker))fail(`summon cinematic missing ${marker}`);
for(const removed of ['bb-paint-stroke','bb-paint-accent','reveal-brush','card-spin-orbit','reveal-resolve-ring','bbBrush','bbCircleCharge','bb-flip-afterimage','rotateY(540deg)','rotateY(720deg)','bbItachiRingSnap','bbItachiRingEngage','bbItachiRingTripleSnap','bbItachiRingDoubleSnap','bbItachiRingSingleSnap','bbItachiRingSlowSnap','bbItachiRingSmoothIn','bbItachiRingFluidIn','bbItachiReference','bbItachiSpin']){
 if(cinematicJs.includes(removed)||cinematicCss.includes(removed)||itachiCss.includes(removed))fail(`busy summon effect survived: ${removed}`);
}
