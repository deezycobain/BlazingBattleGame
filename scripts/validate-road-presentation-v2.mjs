...
for(const stage of [1,2,3,4,5]){
 ...
 if(!(far>=.82&&far<.98&&near>=.98&&near<=1.02&&mid>far&&mid<near))throw new Error(`Road presentation v2: Stage ${stage} depth curve invalid: ${JSON.stringify({far,mid,near,p})}`);
}
const farScales=C.MAPS.map(map=>Number(map.perspective?.farScale));
if(new Set(farScales.map(value=>value.toFixed(2))).size<4)throw new Error(`Road presentation v2: map-authored depth profiles lost their visual variety: ${JSON.stringify(farScales)}`);
const south=C.MAPS.find(map=>map.key==='south-sac');
if(Number(south?.movement?.horizonY)!==248||Math.abs(C.visualScaleForY(south,248)-.84)>.001||Math.abs(C.visualScaleForY(south,540)-1)>.001)throw new Error('Road presentation v2: South Sac must scale from 0.84 at its playable horizon to 1.00 in the foreground');
...
console.log('Road presentation v2 PASS: five map-authored render-only depth profiles, South Sac horizon scaling, brush countdown timing, camera transition/outro, and pre-FIGHT regression coverage are wired.');
