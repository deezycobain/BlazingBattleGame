...
for(const layout of ['centered','riverbank','mountain']){
 ...
 if(!spec||!Array.isArray(spec.center)||spec.center.length!==2)throw new Error(`Sanctuary stone layout metadata missing for ${layout}`);
}
for(const file of [
...
console.log(`Sanctuary validation PASS: integrated root island, locked scene proportions, dedicated grounding/depth layers, natural rake grooves, side-framed grounded stones, canonical compositor, schema V3 stage care, design-driven Harmony, sequenced ceremony FX, Grove showcase, and isolated Seal summon.`);
