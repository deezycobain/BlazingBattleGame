...
def full_card_presentation_meta(card_path: Path):
    """Describe the canonical full card used by presentation UIs without creating a crop."""
...
for unit_id, source_dir in sorted(unit_sources.items()):
    ...
    retired_crop = card_dir / "legacy_summon_art.png"
    if retired_crop.exists():
        retired_crop.unlink()
    presentation_meta = full_card_presentation_meta(card_dir / card_name)
    ...
    data["assets"]["summon_art"] = f"cards/{card_name}"
...
def apply_wong_refresh(audit_units, packages):
    ...
    retired_crop = canonical / "cards" / "legacy_summon_art.png"
    if retired_crop.exists():
        retired_crop.unlink()
    presentation_meta = full_card_presentation_meta(canonical_card)
    data["assets"]["summon_art"] = data["assets"]["art"]
    data["assets"]["team_art"] = data["assets"]["art"]
